...
public class GuiHelper {
    ...
    public static void decorateAsImageButton(JButton button, Image image, Image disabledImage, Color foreground) {
        ...
        button.setDisabledIcon(disabledImage == null
                ? null : new ImageIcon(scaleImage(disabledImage, size.width, size.height)));
...
//    }
    ...
    public static String getOptionText(int optionType) {
        switch (optionType) {
    ...
    public static JButton createButton(int optionType) {
        JButton button = new JButton(getOptionText(optionType));
        Dimension size = new Dimension(70, 25);
        button.setSize(size);
        button.setPreferredSize(size);
        button.setMinimumSize(size);
        button.setMaximumSize(size);
        decorateAsImageButton(button, Images.getButtonImage(), null, Color.white);
        return button;
    }

    public static JButton[] createButtons(int optionsType) {
        switch (optionsType) {
    ...
    public static Rectangle getAreaToFill(Dimension area, Dimension image, boolean uniform, boolean fill) {
        if (uniform) {
            ...
            return fill ? new Rectangle(0, 0, area.width, area.height) : centerRelativeTo(area, image);
        }
    }

    public static Rectangle centerRelativeTo(Dimension area, Dimension image) {
        int x = (area.width - image.width) / 2;
        int y = (area.height - image.height) / 2;
